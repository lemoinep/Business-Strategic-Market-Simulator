...
This program application provides an interactive web dashboard to monitor and simulate an investment portfolio using the Sun Tzu + chess‑inspired AI engine from the `business_sim` core package. It loads a configurable portfolio (see `dash_app/portfolio_config.yaml`), fetches historical market data from Yahoo Finance, and displays live charts of individual tickers and overall portfolio value versus a benchmark index. 

<p align="center">
<img src="Images/P0003.jpg" width="100%" />
</p>

The dashboard also exposes the AI logic in a **decision‑oriented interface**, with live strategic snapshots, automated recommendations, and a structured trade log that can be analyzed or reused for reinforcement learning experiments. 

### Main views

The Dash app is organized into three main tabs: 

- **Live & AI Snapshot**  
  Displays the current market data for the configured tickers, a chart of the portfolio value versus a benchmark (e.g. SPY), and a strategic snapshot generated by `SunTzuChessMarketAI`. The snapshot reports the current phase (Opening, Middlegame, Endgame, Stability), risk tension, center control over key assets (e.g. AAPL, GOOG, MSFT), and the list of recommended tactics (Attack / Defense / Simplification). 

- **AI Simulation (N turns)**  
  Runs a multi‑turn simulation of the portfolio using the same strategic engine, with configurable number of turns and initial cash. It plots the simulated portfolio trajectory and a simulated market index, and reports summary metrics such as how many turns beat the market and the maximum drawdown over the simulation horizon. This tab is useful to explore how different Sun Tzu configurations might behave under varying market conditions. 

- **Trade History & Analytics**  
  Reads a CSV trade log (e.g. `data/trade_history.csv`) and shows a sortable table of all executed trades together with a summary panel and a bar chart of trade types. The summary includes total number of orders, buy/sell volumes, and counts of risk‑management trades, profit‑taking trades, and drawdown‑control trades. This provides a quick view of how often the AI and the user take profits, cut losses, or reduce oversized positions. 

### Decision Assistant and execution

A central feature of the dashboard is the **Decision Assistant**, which turns the AI snapshot into concrete, human‑readable trade recommendations and lets you execute them in a simulated portfolio. 

- The assistant generates a structured recommendation with:  
  - a textual explanation (“I recommend adjusting your portfolio as follows: …”),  
  - a list of actions (ticker, side buy/sell, quantity, reason). 

- Recommendations combine several layers of logic:  
  - **Tactics derived from Sun Tzu + chess** (e.g. “[ATTACK] Strengthen NVDA, central market control (>55%)”),  
  - **Risk management rules** (reducing oversized positions above a configurable weight threshold),  
  - **Profit‑taking and drawdown‑control rules** based on each position’s average cost (`avg_price`) and configurable profit/drawdown thresholds. 

- An **“Execute decision”** button applies the actions to the in‑memory portfolio (`POSITIONS`) in simulation mode. Quantities and average prices are updated using an average‑cost method, and each executed action is appended to the CSV trade history with its full context (timestamp, mode, ticker, side, quantity, price, phase, tension, center control, reason). 

Execution can be done in manual or “auto” mode: in manual mode the user validates each decision explicitly; in auto mode the button executes immediately without confirmation. This is designed to mirror how a real‑world assistant might transition from advisory to semi‑automatic trading. 

### Risk management controls

The Decision Assistant includes a dedicated **risk management control panel** with sliders that let you tune the behavior of the sell logic in real time: 

- **Max position weight (%)**  
  Sets the maximum allowed weight for any single position in the portfolio (e.g. 25%). Positions above this threshold generate “Risk management: reduce oversized position (…)” sell actions that trim the excess exposure. 

- **Profit threshold (%)**  
  Defines the minimum gain from cost basis (`avg_price`) at which the assistant starts proposing partial profit‑taking trades (e.g. +20%). When the current price is sufficiently above `avg_price`, the assistant suggests small sells tagged as “Profit taking: position up … from cost basis (threshold=…)”. 

- **Drawdown threshold (%)**  
  Defines the maximum tolerated loss from cost basis before proposing defensive sells (e.g. –15%). Positions whose performance falls below this threshold generate actions like “Drawdown control: position down … from cost basis (threshold=…)”. 


A **“Reset to defaults”** button instantly restores the sliders to a standard configuration (e.g. 25% max weight, 20% profit threshold, –15% drawdown threshold), making it easy to experiment and then return to a baseline risk profile. 

These controls allow you to move seamlessly between more conservative and more aggressive behaviors, while keeping the underlying Sun Tzu + chess tactical engine unchanged.

### Trade history, analytics and RL integration

Every executed decision is stored in a CSV trade log with rich metadata, which the dashboard uses for analytics and which can be reused for reinforcement learning or offline research. 

- The **Trade History** tab summarizes:  
  - total number of trades and total buy/sell volumes,  
  - counts and percentages of profit‑taking trades, drawdown‑control trades, and risk‑management trades,  
  - a bar chart showing the distribution of trade types (Profit taking / Drawdown control / Risk management / Other). 

- The detailed table allows inspection of each trade, including:  
  - strategic phase (Opening/Middlegame/Endgame/Stability),  
  - risk tension and center control at the time of execution,  
  - textual reason (e.g. tactical recommendation vs risk‑management rule). 

Because the log is a simple CSV file with consistent columns, it can be easily consumed by external tools (Python notebooks, RL training scripts, quantitative backtesters) to compare the Sun Tzu strategy with other agents, or to train models that learn from these decisions. This connects the interactive dashboard to the broader **RL/HPC experimentation pipeline** provided by the rest of the repository.
