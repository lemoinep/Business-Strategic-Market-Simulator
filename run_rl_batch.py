# Batch generator for RL trajectories using PortfolioEnv

import csv
import random

from tqdm import tqdm

from business_sim.core_portfolio import PortfolioEnv


def random_policy(n_assets: int):
    """Simple random allocation over n_assets (uniform Dirichlet-like)."""
    x = [random.random() for _ in range(n_assets)]
    s = sum(x)
    if s <= 0:
        return [1.0 / n_assets] * n_assets
    return [v / s for v in x]


def run_batch(
    num_episodes: int = 100,
    max_turns: int = 30,
    tickers=None,
    cash: float = 10000,
    out_csv: str = "rl_batch_data.csv",
):
    rows = []

    for ep in tqdm(range(num_episodes), desc="Generating episodes"):
        env = PortfolioEnv(max_turns=max_turns, tickers=tickers, cash=cash)
        state = env.reset()
        done = False
        step_idx = 0

        while not done:
            n_assets = len(env.state_obj.assets)
            action = random_policy(n_assets)

            next_state, reward, done, info = env.step(action)
            step_idx += 1

            row = {
                "episode": ep,
                "step": step_idx,
                "reward": reward,
                "done": int(done),
            }

            # Full state vector
            for i, s_val in enumerate(next_state):
                row[f"state_{i}"] = s_val

            # Main info fields
            row["cash"] = info.get("cash", 0.0)
            row["portfolio_value"] = info.get("portfolio_value", 0.0)
            row["market_fear"] = info.get("market_fear", 0.0)
            row["market_liquidity"] = info.get("market_liquidity", 0.0)
            row["market_volatility"] = info.get("market_volatility", 0.0)
            row["center_control"] = info.get("center_control", 0.0)
            row["risk_tension"] = info.get("risk_tension", 0.0)
            row["ai_personality"] = info.get("ai_personality", "")
            row["ai_phase"] = info.get("ai_phase", "")
            row["turn"] = info.get("turn", step_idx)

            # Applied action
            for i, a in enumerate(action):
                row[f"action_{i}"] = a

            rows.append(row)
            state = next_state

    if rows:
        fieldnames = list(rows[0].keys())
        with open(out_csv, "w", newline="", encoding="utf-8") as f:
            writer = csv.DictWriter(f, fieldnames=fieldnames)
            writer.writeheader()
            writer.writerows(rows)

    return rows


if __name__ == "__main__":
    print("Starting RL batch generation...")
    try:
        run_batch(
            num_episodes=10,
            max_turns=5,
            tickers=["NVDA", "AMD", "MSFT", "SPY"],
            cash=20000,
            out_csv="rl_batch_data.csv",
        )
        print("Done.")
    except Exception as e:
        print("Error during batch generation:", e)